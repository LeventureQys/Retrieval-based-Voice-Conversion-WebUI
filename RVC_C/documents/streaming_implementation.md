...
| `src/rvc_streaming.cpp` | ⏳ 待实现 | 流式处理实现 |
| `tests/test_streaming.cpp` | ⏳ 待实现 | 测试程序 |

### 需修改文件

| 文件 | 状态 | 说明 |
|------|------|------|
| `CMakeLists.txt` | ⏳ 待修改 | 添加新源文件 |
...
添加 `src/rvc_streaming.cpp` 到源文件列表。

### 3. 创建测试程序

`tests/test_streaming.cpp` 应包含：
- 单块处理测试
- 多块连续处理测试
- 边界条件测试
